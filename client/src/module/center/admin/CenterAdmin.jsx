...
import { Avatar, Button, Card, Grid, Input, Popconfirm, Space, Tag, Tooltip, Typography, message } from 'antd';
import {
  PlusOutlined,
  SearchOutlined,
  EditOutlined,
...
import { useDeleteAdminMutation, useLazyGetAdminsQuery } from '../../../service/center/ApiAdmin';
...
const CenterAdmin = () => {
  ...
  const refreshFirstPage = async () => {
    try {
      const result = await triggerGetAdmins({ page: 1, limit: 10, search }, true).unwrap();
  ...
  const renderItem = (item) => {
    ...
    return (
      <MotionDiv key={item.id} variants={itemVariants} whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
        <Card
          variant="borderless"
          hoverable
          actions={[
            ...
            <Space orientation="vertical" size={10} style={{ flex: 1, minWidth: 0 }}>
              <div>
                <Title
                  ...
                  {item.full_name}
                  {isCenter ? <CrownFilled style={{ color: '#f59e0b', marginLeft: 8 }} /> : null}
                </Title>
                <Text
                  type="secondary"
            ...
            <Space orientation="vertical" size={14} style={{ width: '100%' }}>
              ...
              <div>
                ...
                <Text
                  ...
                  Cari akun, perbarui akses, dan tambah admin baru dari satu tampilan yang tetap nyaman dipakai di

...
import { spmbNavActions, spmbNavSections } from "./nav-config.js";
...
export function SpmbAdminSidebar({ collapsed, onToggleCollapse, schoolName, logoUrl, hasLogo }) {
  ...
  return (
    <aside
      ...
      aria-label="Navigasi admin SPMB"
    >
      <div
        ...
        >
          ...
          {!collapsed ? (
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-slate-400">Admin SPMB</p>
            </div>
          ) : null}
        </Link>
      ...
      <div className="shrink-0 border-t p-3" style={{ borderColor: "var(--admin-sidebar-border)" }}>
        {collapsed ? (
          <div className="flex flex-col gap-1">
            {spmbNavActions.map(renderNavAction)}
            <button
              type="button"
              onClick={onToggleCollapse}
